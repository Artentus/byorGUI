...
impl<U: Unit> Vec2<U> {
    #[inline]
    fn along_axis(self, axis: Axis) -> Float<U> {
        match axis {
            Axis::X => self.x,
            Axis::Y => self.y,
        }
    ...
    #[inline]
    fn along_axis_mut(&mut self, axis: Axis) -> &mut Float<U> {
        match axis {
            Axis::X => &mut self.x,
            Axis::Y => &mut self.y,
        }
...
impl ComputedPadding {
    #[inline]
    fn along_axis(&self, axis: Axis) -> [Float<Pixel>; 2] {
        match axis {
...
impl ComputedStyle {
    #[inline]
    fn size_along_axis(&self, axis: Axis) -> ComputedSizing {
        match axis {
            Axis::X => self.width(),
            Axis::Y => self.height(),
        }
